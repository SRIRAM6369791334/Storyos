...
```mermaid
sequenceDiagram
    participant C as Chaos Runner
    participant K as K8s API
    participant P as Target Pod
    C->>K: Submit ChaosEngine CRD
    K->>P: Inject Fault
    P-->>C: Metrics (SLO evaluation)
    C->>K: Revert Fault
```

## 7. Data Flow
- CRD Submission -> Operator -> Fault Injector -> Target

## 8. Runtime Lifecycle
```mermaid
stateDiagram-v2
    [*] --> Steady
    Steady --> FaultInjection
    FaultInjection --> Evaluation
    Evaluation --> Abort : SLO Breach
    Evaluation --> Steady : OK
```

## 9. Security Architecture
| Control ID | Description | Enforcement |
|---|---|---|
| SEC-CHAOS-001 | RBAC | Kubernetes |
...
## 10. Scalability
Operator scales dynamically.

## 11. Reliability
| SLI | SLO | Alert Threshold | Escalation |
|---|---|---|---|
| Experiment Success | 99.99% | < 99.95% | SRE |

## 12. Performance
| Operation | P50 | P95 | P99 | Throughput |
|---|---|---|---|---|
| Inject | 100ms | 300ms | 1000ms | 100 |

## 13. Observability
```text
storyos_chaos_experiments_total 5
```

## 14. Failure Handling
1. Alert triggers abort.
2. Operator restores state.

## 15. Testing Strategy
- Chaos-1: Pod Kill
- Chaos-2: Network Partition
- Sec-1: Unauthorized RBAC
- Sec-2: SSRF

## 16. Governance Rules
### CHAOS-001
- Rule: Auto abort required
- Rationale: Prevent cascading failure
- Enforcement: Admission controller

## 17. Cross-Document Integration
| Doc | Relationship | Shared |
|---|---|---|
| REL | Reliability | SLO Metrics |

## 18. Future Evolution
Move to continuous chaos in production.

## 19. Executive Summary
Chaos architecture ensures resilience.

## Required Technical Artifacts
### PostgreSQL Schema
```sql
CREATE SCHEMA chaos_data;
CREATE TABLE chaos_data.config (
    id UUID,
    tenant_id UUID,
    val TEXT
);
